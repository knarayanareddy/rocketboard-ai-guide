...
DECLARE
  ...
  tsq tsquery;
BEGIN
  -- Build tsquery: try AND first, fall back to OR if AND returns nothing
  tsq := plainto_tsquery('english', fts_query);

  IF p_query_embedding IS NULL THEN
    -- Check if AND query matches anything
    IF NOT EXISTS (
      SELECT 1 FROM public.knowledge_chunks kc
      WHERE kc.pack_id = p_pack_id AND kc.is_redacted = false AND kc.fts @@ tsq
      LIMIT 1
    ) THEN
      -- Fall back to OR-style: split words and join with |
      tsq := array_to_string(
        ARRAY(SELECT unnest(string_to_array(fts_query, ' ')) ),
        ' | '
      )::tsquery;
      -- If that fails to parse, use a simple approach
      BEGIN
        -- no-op, tsq is already set
    ...
    SELECT
      ...
      ts_rank(kc.fts, tsq)::double precision AS score
    ...
    WHERE kc.pack_id = p_pack_id
      ...
      AND (p_match_threshold IS NULL OR ts_rank(kc.fts, tsq)::double precision >= p_match_threshold)
    ORDER BY ts_rank(kc.fts, tsq) DESC, kc.created_at DESC
    LIMIT safe_count;
    RETURN;
  END IF;
