-- Migration 2: Harden KG Expansion RPC
-- Adds org_id filtering, symbol sanitization, null-safe scoring

CREATE OR REPLACE FUNCTION public.kg_expand_v1(
  p_org_id uuid, p_pack_id uuid, p_seed_ids uuid[],
  p_symbols text[] DEFAULT NULL, p_limit int DEFAULT 15, p_max_per_relation int DEFAULT 5
)
RETURNS TABLE (
...
BEGIN
  ...
  FROM public.pack_active_generation WHERE org_id = p_org_id AND pack_id = p_pack_id;

  IF v_generation_id IS NULL THEN
    v_generation_id := (SELECT kc.generation_id FROM public.knowledge_chunks kc
      WHERE kc.org_id = p_org_id AND kc.pack_id = p_pack_id ORDER BY kc.created_at DESC LIMIT 1);
  END IF;

  WITH seed_metadata AS (
    SELECT exported_names, imports FROM public.knowledge_chunks
    WHERE id = ANY(p_seed_ids) AND org_id = p_org_id AND pack_id = p_pack_id
  ...
  IF array_length(v_all_symbols, 1) > 50 THEN v_all_symbols := v_all_symbols[1:50]; END IF;
  ...
  WITH expanded_nodes AS (
    ...
    WHERE kc.org_id = p_org_id AND kc.pack_id = p_pack_id
      ...
      AND kc.is_redacted = false AND kc.exported_names && ARRAY[sym] AND kc.id != ANY(p_seed_ids))
    UNION ALL
    (SELECT kc.id, kc.chunk_id, kc.source_id, kc.path, kc.content,
      kc.entity_type, kc.entity_name, kc.signature, kc.start_line, kc.end_line,
    ...
    WHERE kc.org_id = p_org_id AND kc.pack_id = p_pack_id
      AND (v_generation_id IS NULL OR kc.generation_id = v_generation_id)
      AND kc.is_redacted = false AND kc.imports && ARRAY[sym] AND kc.id != ANY(p_seed_ids))
    UNION ALL
    (SELECT kc.id, kc.chunk_id, kc.source_id, kc.path, kc.content,
      kc.entity_type, kc.entity_name, kc.signature, kc.start_line, kc.end_line,
    ...
    FROM public.knowledge_chunks kc JOIN public.knowledge_chunks seed ON seed.id = ANY(p_seed_ids)
    WHERE kc.org_id = p_org_id AND kc.pack_id = p_pack_id
      AND (v_generation_id IS NULL OR kc.generation_id = v_generation_id)
      AND kc.is_redacted = false AND kc.path = seed.path
      AND ABS(kc.start_line - seed.start_line) < 200 AND kc.id != ANY(p_seed_ids))
  ),
  scored_nodes AS (
    SELECT en.id, en.chunk_id, en.source_id, en.path, en.content,
      ...
      en.r_type, en.r_sym,
      (en.base_score * CASE
        WHEN en.r_sym IS NOT NULL AND en.r_sym = ANY(COALESCE(p_symbols, '{}')) THEN 1.0
        ...
        ) THEN 0.5 ELSE 1.0 END) as final_score
    FROM expanded_nodes en WHERE en.rank <= v_max_per
  )
  SELECT DISTINCT ON (sn.id) sn.id, sn.chunk_id, sn.source_id, sn.path, sn.content,
