-- Migration: KG Retrieval v2 Expansion
-- Description: Adds GIN index for imports and implements the kg_expand_v1 deterministic graph expansion RPC.

-- 1. Add missing index forimports
  ...
  WITH expanded_nodes AS (
    ...
    (
      ...
      WHERE kc.pack_id = p_pack_id
        ...
        AND kc.id != ANY(p_seed_ids)
        AND kc.id NOT IN (SELECT id FROM public.knowledge_chunks WHERE id = ANY(p_seed_ids))
    )
  ),
  scored_nodes AS (
    SELECT 
      ...
      (en.base_score * 
       CASE 
         WHEN en.r_sym IS NOT NULL AND en.r_sym = ANY(p_symbols) THEN 1.0  -- Priority for user-provided symbols
