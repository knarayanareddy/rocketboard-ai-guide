-- Fix hybrid_search_v2: Replace to_tsquery with websearch_to_tsquery for natural language support
-- Also lower the default match threshold to 0.3 for better recall in broad tasks.
...
CREATE OR REPLACE FUNCTION hybrid_search_v2(
  ...
  p_match_threshold FLOAT DEFAULT 0.3, -- Lowered from 0.5
...
BEGIN
  ...
  fts_matches AS (
    ...
    WHERE kc.org_id = p_org_id 
      ...
      AND (p_module_key IS NULL OR kc.module_key = p_module_key)
      AND (p_track_key IS NULL OR kc.track_key = p_track_key)
      AND kc.fts @@ websearch_to_tsquery('simple', p_query_text) -- Changed to websearch_to_tsquery
    LIMIT p_match_count * 2
  ),
  rrf_combined AS (
    SELECT 
