...
DROP FUNCTION IF EXISTS public.hybrid_search_v2_impl(uuid, uuid, text, vector, integer, double precision, text, text);
...
BEGIN
  IF p_query_embedding IS NULL THEN
    ...
    SELECT
      ...
      ts_rank(kc.fts, websearch_to_tsquery('english', p_query_text))::double precision AS score
    ...
    WHERE kc.pack_id = p_pack_id
      ...
      AND (p_match_threshold IS NULL OR ts_rank(kc.fts, websearch_to_tsquery('english', p_query_text))::double precision >= p_match_threshold)
    ORDER BY ts_rank(kc.fts, websearch_to_tsquery('english', p_query_text)) DESC, kc.created_at DESC
    LIMIT safe_count;
    RETURN;
  END IF;
  ...
  fts_results AS (
    SELECT
      ...
      ROW_NUMBER() OVER (ORDER BY ts_rank(kc.fts, websearch_to_tsquery('english', p_query_text)) DESC) AS fts_rank
    FROM public.knowledge_chunks kc
    WHERE kc.pack_id = p_pack_id
      AND kc.is_redacted = false
      AND kc.fts @@ websearch_to_tsquery('english', p_query_text)
      AND (p_module_key IS NULL OR COALESCE(kc.metadata->>'module_key', '') = p_module_key)
      AND (p_track_key IS NULL OR COALESCE(kc.metadata->>'track_key', '') = p_track_key)
    ORDER BY ts_rank(kc.fts, websearch_to_tsquery('english', p_query_text)) DESC
    LIMIT safe_count * 3
  ),
  combined AS (
    SELECT
