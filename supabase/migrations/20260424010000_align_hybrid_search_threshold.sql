...
CREATE OR REPLACE FUNCTION public.hybrid_search_v2(
  ...
  p_match_threshold FLOAT DEFAULT 0.3, -- Reverted to 0.3 per 20260326000000_fix_hybrid_search_nl.sql
...
DECLARE
  ...
  v_match_count INT;
BEGIN
  -- 1. WHY: This aligns with the NL fix which lowered the threshold to 0.3 to increase recall.
  -- 2. COMPARATOR: Using `>=` ensures that chunks exactly matching the threshold are included,
  -- maintaining parity with standard vector search implementations and the previous logic.

  -- Defense in Depth: Verify membership
  ...
  WITH vector_matches AS (
    ...
    WHERE p_query_embedding IS NOT NULL
      ...
      -- Using >= for inclusive matching/consistency
