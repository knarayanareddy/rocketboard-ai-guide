 ...
 */
export function chunkMarkdownByHeadings(markdown: string, options: ChunkOptions = {}): { start: number; end: number; text: string }[] {
 ...
 */
export function fallbackChunkWords(text: string, options: ChunkOptions = {}): { start: number; end: number; text: string }[] {
  const { maxWords = 500, overlapWords = 50 } = options;
