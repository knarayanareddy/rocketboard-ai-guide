...
export async function getSourceCredential(
  ...
  if (error) {
    console.error(`[CREDENTIALS] Failed to read credential for source ${packSourceId}:`, error.message);
    return null;
  }
...
export async function storeSourceCredential(
  ...
  if (error) {
    console.error(`[CREDENTIALS] Failed to store credential for source ${packSourceId}:`, error.message);
    return null;
  }
...
export async function deleteSourceCredential(
  ...
  if (error) {
    console.error(`[CREDENTIALS] Failed to delete credential for source ${packSourceId}:`, error.message);
    return false;
  }
