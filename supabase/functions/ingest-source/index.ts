...
async function getSourceCredential(serviceClient: any, sourceId: string, key: string): Promise<string | null> {
...
async function initializeIngestion(
  ...
  try {
    if (source_type === "github_repo") {
      ...
      const { error: stateErr } = await serviceClient.from("ingestion_job_state").insert({
        job_id: jobId,
        pack_id,
        source_id,
        files_json: files,
      ...
      const docChunks = chunks.map((c, i) => {
        ...
        return {
          pack_id, source_id, org_id, generation_id: jobId,
      ...
      const { error: upsertErr } = await serviceClient.from("knowledge_chunks").upsert(docChunks);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { pack_id, source_id, source_type, source_uri, document_content, label, source_config } = body;

    const { userId } = await requireUser(req, corsHeaders);
    const serviceClient = createServiceClient();
    await requirePackRole(serviceClient, pack_id, userId, "author", corsHeaders);

    const { data: packRow } = await serviceClient.from("packs").select("org_id").eq("id", pack_id).single();
    const effectiveOrgId = packRow?.org_id;

    const { data: job, error: jobErr } = await serviceClient.from("ingestion_jobs").insert({ 
      pack_id, source_id, status: "processing", started_at: new Date().toISOString() 
    }).select("id").single();
    ...
    const trace = createTrace({
      ...
      packId: pack_id, sourceId: source_id, orgId: effectiveOrgId,
      environment: Deno.env.get("ENVIRONMENT") || "production",
    }, { enabled: shouldTrace() });
    trace.updateMetadata({ jobId });

    const task = initializeIngestion(serviceClient, jobId, pack_id, source_id, source_type, source_uri, document_content, label, effectiveOrgId, functionUrl);
