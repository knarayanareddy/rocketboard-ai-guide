...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const allUniqueChunkIds = Array.from(new Set(
      Array.from(sectionReferences.values()).flatMap(set => Array.from(set))
    ));

    if (allUniqueChunkIds.length === 0) {
      console.log(`[FRESHNESS] No citations found for module ${module_key}. Skipping ledger recording.`);
      return json(200, { message: "No citations found", tracked_sections: 0 }, corsHeaders);
    }
    ...
    const hashMap = new Map((chunks || []).map(c => [c.chunk_id, c.content_hash]));
    ...
    for (const [sectionId, chunkIds] of sectionReferences.entries()) {
      ...
      if (error) {
        console.error(`[FRESHNESS] Failed to upsert for section ${sectionId}:`, error);
        continue;
      }
      results.push(data.id);
    }

    console.log(`[FRESHNESS] Recorded ledger entries for ${results.length} sections in module ${module_key}.`);
