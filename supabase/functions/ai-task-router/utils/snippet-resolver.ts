...
export function resolveSnippets(text: string, spans: EvidenceSpan[]) {
  const snippetRegex = /\[SNIPPET:\s*([^\]:]+?):\s*(\d+)-(\d+)\s*\|\s*lang=(.*?)\s*\]/g;
  let snippets_resolved = 0;

  const finalMarkdown = text.replace(snippetRegex, (match, path, startStr, endStr, lang, offset) => {
    ...
    return "\n```" + lang.trim() + "\n// SOURCE: " + normalizedPath + ":" + start + "-" + end + "\n" + extracted + "\n```\n";
  });
