...
export type SupportedLanguage = "typescript" | "javascript" | "python" | "go" | "java" | "rust" | string;

const STOPLISTS: Record<string, Set<string>> = {
  typescript: new Set(["then", "catch", "map", "filter", "reduce", "Object", "String", "Number", "Promise", "console", "log"]),
  javascript: new Set(["then", "catch", "map", "filter", "reduce", "Object", "String", "Number", "Promise", "console", "log"]),
  go: new Set(["fmt", "context", "error", "err", "len", "make", "new"]),
  python: new Set(["self", "cls", "len", "dict", "list", "str", "int", "print"]),
  java: new Set(["String", "Object", "List", "Map", "Optional", "System", "out", "println"]),
  rust: new Set(["Result", "Option", "unwrap", "expect", "println"]),
  common: new Set(["if", "else", "for", "while", "return", "class", "function", "const", "let", "var", "import", "export", "from", "as", "default", "async", "await", "try", "catch", "throw", "new", "this", "super", "extends", "implements", "interface", "type", "enum", "public", "private", "protected", "static", "readonly", "string", "number", "boolean", "any", "void", "null", "undefined", "true", "false", "item", "data", "id", "name", "type", "uuid", "text", "content"]),
};

/**
 * Detects if a symbol is a language-specific or common stopword.
 */
export function isStopSymbol(sym: string, languageHint?: SupportedLanguage): boolean {
 ...
 */
export function normalizeCandidateSymbols(rawSymbols: string[], languageHint?: SupportedLanguage): string[] {
  ...
  return Array.from(result).filter(s => !isStopSymbol(s, languageHint) && s.length >= 3);
}

/**
 * Improved deterministic extraction from text.
 */
export function extractQualifiedSymbolsFromText(text: string, languageHint?: SupportedLanguage): string[] {
