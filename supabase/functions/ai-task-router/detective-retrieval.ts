...
export async function runDetectiveRetrieval(
  ...
  while (hopsRun < options.maxHops && Date.now() - startTime < options.maxTimeMs) {
    hopsRun++;
    ...
    // 2. Extract symbols
    const symbols = await extractCandidateSymbols(keptForSymbols, supabase, packId, options.maxSymbols);
    symbolsExtractedCount = symbols.length;

    if (symbols.length === 0) break;

    // 3. Hop 1 Retrieval (Definitions)
    const { data: hop1Spans, error: hop1Error } = await supabase.rpc("definition_search_v1", {
      p_org_id: orgId,
      p_pack_id: packId,
      p_symbols: symbols,
      p_match_count: 15,
  ...
  for (const span of spans) {
    ...
    if (span.chunk_id) {
       ...
       if (chunk) {
         if (chunk.exported_names) {
           const normalized = normalizeCandidateSymbols(chunk.exported_names, lang);
    ...
    if (text) {
      ...
      extracted.forEach(name => {
        ...
        let weight = 1;
        if (text.includes(`class ${name}`) || text.includes(`interface ${name}`) || text.includes(`type ${name}`)) {
          weight = 2;
        }
